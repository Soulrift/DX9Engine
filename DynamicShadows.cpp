...
DynamicShadows* DynamicShadows::Instance()
{
	static DynamicShadows temp;
	return &temp;
}

void DynamicShadows::InitializeState(Application* owner)
{
	...
	*/
	...
	m_bFXLog = 0;
	HRESULT hr;
	hr = D3DXCreateEffectFromFile(gd3dDevice, "DynShadow.fx", 0, 0, D3DXSHADER_DEBUG, 0, &mFX, &m_bFXLog);
	if( m_bFXLog )
	...
	mhLightPos           = mFX->GetParameterByName(0, "gLightPos");
...
void DynamicShadows::UpdateScene(Application* owner, double dT)
{
...
void DynamicShadows::RenderScene(Application* owner)
{
	...
	{
		//cout << "blah" << endl;
		gd3dDevice->SetRenderTarget(0, pImposterSurface);
		gd3dDevice->Clear(0, NULL,  D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB(0,1,1,1), 1.0f, 0);
		gd3dDevice->BeginScene();
		
		m_camPosition = m_Cam1->GetPos();
		mFX->SetTechnique(m_hImpTechnique);
	...
	mFX->Begin(NULL, NULL);
		mFX->BeginPass(0);
			...
			//Draw Imposter
			mFX->SetMatrix(mhLightWVP, &(m_oImposterQuad->m_mWorld * mLightVP));
			mFX->SetMatrix(mhWVP, &(m_oImposterQuad->m_mWorld * m_Cam1->GetViewProj()));
			mFX->SetMatrix(mhWorld, &m_oImposterQuad->m_mWorld);
			mFX->SetValue(mhMtrl, &mBaseMaterial, sizeof(D3DMATERIAL9));
			if (hasImposter)
			{
				//cout << "blah" << endl;
				mFX->SetTexture(mhTex, pImposterTexture);
			}
			else
				mFX->SetTexture(mhTex, m_oImposterQuad->m_pTexture->GetTexture());
			mFX->CommitChanges();
			m_oImposterQuad->Render();
		mFX->EndPass();
	mFX->End();
	/*
	...
	mFX->Begin(NULL, NULL);
		mFX->BeginPass(0);
			//Draw the dwarf's shadow
			mFX->SetValue(mhLightPos, &lPointLight.m_vPosition, sizeof(D3DXVECTOR3));
			mFX->SetMatrix(mhWVP, &(m_mWorld2 * m_Cam1->GetViewProj()));
			mFX->SetMatrix(mhWorld, &m_mWorld2);
			mFX->SetMatrix(mhViewProj, &m_Cam1->GetViewProj());
...
void DynamicShadows::OnResetDevice(Application* owner)
{
	//update perspective
	float w = (float)owner->getParam().BackBufferWidth;
	float h = (float)owner->getParam().BackBufferHeight;
...
void DynamicShadows::OnLostDevice(Application* owner)
{
	//m_eEffect->OnLostDevice();
}

void DynamicShadows::LeaveState(Application* owner)
{
	//m_eEffect->Release();
